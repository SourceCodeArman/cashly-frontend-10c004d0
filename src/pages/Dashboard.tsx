              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    {data?.goals.filter(g => g.is_active).length || 0} goals in progress
                  </p>
                </div>
              </div>
              <div className="text-right">
                <p className="text-xl font-bold text-accent">
                  {formatCurrency(data?.goals.reduce((sum, g) => sum + (g.current_amount || 0), 0) || 0)}
                </p>
                <p className="text-sm text-muted-foreground">
                  of {formatCurrency(data?.goals.reduce((sum, g) => sum + g.target_amount, 0) || 0)}
                </p>
              </div>
            </div>
          </CardContent>
